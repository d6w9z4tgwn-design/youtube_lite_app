...
def confirm_zundamon_terms():
    print("\nずんだもん（zundamon-1）モデルを外部配布元から取得します。", flush=True)
    print("このモデルはClipNestやこのGitHubリポジトリには同梱されていません。", flush=True)
    print("利用前に、以下のモデル配布元・関連する利用条件を確認してください。", flush=True)
    print(f"モデル配布元: {MODEL_CARD}", flush=True)
    print(f"利用条件: {MODEL_TERMS}", flush=True)
    print("利用条件への適合性は利用者自身で判断してください。", flush=True)
    answer = input("利用条件を確認し、自身の責任で使用することに同意しますか？ [y/N]: ").strip().lower()
    if answer not in {"y", "yes"}:
        raise SystemExit("同意されなかったため、ずんだもんモデルの取得を中止しました。")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--zundamon-personal', action='store_true', help='Install zundamon-1 after explicit terms confirmation')
